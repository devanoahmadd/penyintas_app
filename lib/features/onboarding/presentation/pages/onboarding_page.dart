        ...
        _stagger(
          _DockedPanel(
            ...
            keypad: OnboardingKeypad(
              isDark: isDark,
              onKey: (k) => setState(() {
                // #212: first digit after preset taps starts a fresh number
                if (_incomePresetSelected && k != 'back') {
                  _income = applyOnboardingKey(0, k);
                  ...
                  Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          child: _ExtremChip(
                            // #209: active only when exactly 25 (not ≥20)
                            active: _pct == 25,
                            // #201: use l10n key for "Ekstrem" label
                            label: l.onboardingPctLabelMax,
                            isDark: isDark,
                            surfaceAlt: surfaceAlt,
...
class _PaydayChipWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // #203: min 44dp tap target; #204: button semantics
    return Semantics(
      button: true,
      label: label,
      selected: active,
...
class _ExtremChip extends StatelessWidget {
  const _ExtremChip({
    ...
    required this.surfaceAlt,
    required this.onTap,
    required this.label, // #201: l10n label injected from parent
  });
...
class _OSliderState extends State<_OSlider> {
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(
      slider: true,
      value: '${widget.value}%',
      increasedValue: '${math.min(_max, widget.value + 1)}%',
