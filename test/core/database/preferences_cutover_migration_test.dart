...
void _seedV10(
  ...
  ''');
  raw.execute("INSERT INTO app_settings (id, locale) VALUES (1, '$appLocale')");
  raw.execute('''
  ...
  ''');
  raw.execute(
    "INSERT INTO preferences (id, language) VALUES (1, '$prefsLang')",
  );
}

void main() {
  test(
    'migrasi 10→11: re-seed preferences.language dari app_settings.locale (cutover)',
    () async {
      ...
      _seedV10(
        ...
        appLocale: 'en',
        prefsLang: 'id',
      ); // DRIFT: app=en, prefs=id

      final db = AppDatabase(
        NativeDatabase.opened(raw),
      ); // buka v11 → 10→11 jalan
      ...
      expect(
        row!.language,
        'en',
        reason: 'language ter-re-seed dari app_settings.locale terbaru',
      );
      await db.close();
    },
  );
  ...
  test('migrasi 10→11: locale liar di app_settings di-CLAMP ke id', () async {
    ...
    expect(
      row!.language,
      'id',
      reason: 'nilai liar tak boleh tersalin verbatim',
    );
    await db.close();
  });

  test(
    'migrasi 10→11: tanpa row app_settings → language preferences TAK berubah',
    () async {
      final raw = sqlite3.openInMemory();
      ...
      expect(
        row!.language,
        'en',
        reason: 'EXISTS-guard: tak meng-UPDATE jadi null/default',
      );
      await db.close();
    },
  );
