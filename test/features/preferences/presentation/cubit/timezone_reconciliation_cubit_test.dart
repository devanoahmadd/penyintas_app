...
void main() {
  ...
  TimezoneReconciliationCubit build(String deviceTz) =>
      TimezoneReconciliationCubit(
        repo: repo,
        tz: tz,
        getDeviceTimezone: () async => deviceTz,
      );

  test('device == tersimpan → tak ada prompt', () async {
    when(() => repo.read()).thenAnswer(
      (_) async => PreferencesEntity.defaults.copyWith(
  ...
  test(
    'device != tersimpan → prompt dgn label device + label tersimpan (currentCity konsisten)',
    () async {
      when(() => repo.read()).thenAnswer(
        (_) async => PreferencesEntity.defaults.copyWith(
          timezone: 'Asia/Jakarta',
      ...
      expect(
        c.state.prompt!.storedLabel,
        'Jakarta',
      ); // F-D5b: kota cocok zona → pakai kota
    },
  );
  ...
  test('getDeviceTimezone throw → tak ada prompt (tak crash launch)', () async {
    ...
    final c = TimezoneReconciliationCubit(
      repo: repo,
      tz: tz,
      getDeviceTimezone: () async => throw Exception('no plugin'),
    );
    await c.check();
    expect(c.state.prompt, isNull);
  });
  ...
  test('dismiss → prompt hilang tanpa menyimpan', () async {
    ...
    expect(
      c.state.prompt,
      isNotNull,
    ); // prompt muncul dulu (device != tersimpan)
    c.dismiss();
    expect(c.state.prompt, isNull);
  ...
  test(
    'F-D5b label basi: currentCity tak cocok timezone → pakai labelForIana, bukan kota basi',
    () async {
      ...
      final c = build(
        'Asia/Jakarta',
      ); // device beda dari Moscow tersimpan → prompt
      await c.check();
      expect(
        c.state.prompt!.storedLabel,
        'Moscow · GMT+3',
  ...
  test(
    'F-D5 snooze: setelah dismiss zona sama → check ulang TAK prompt (anti-nag remount)',
    () async {
      when(() => repo.read()).thenAnswer(
        (_) async =>
            PreferencesEntity.defaults.copyWith(timezone: 'Asia/Jakarta'),
      ...
      c.dismiss();
      await c
          .check(); // simulasi remount dashboard pada instance singleton yang sama
      expect(
        c.state.prompt,
        isNull,
  ...
  test(
    'F-D5c confirm bersihkan snooze: edge travel X→dismiss→Y→confirm→balik X di-prompt lagi',
    () async {
      ...
      final c = TimezoneReconciliationCubit(
        repo: repo,
        tz: tz,
        getDeviceTimezone: () async => device,
      );
