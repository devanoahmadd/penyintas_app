...
void main() {
  ...
  test('lockout aktif → lockedOut tanpa memanggil verifyPin', () async {
    ...
    expect(await verifyPinWithLockout(repo, '123456'),
        PinVerifyOutcome.lockedOut);
    verifyNever(() => repo.verifyPin(any()));
  });

  test('lockout kedaluwarsa (timestamp lampau) → tidak lockedOut, verifyPin tetap dipanggil',
      () async {
    when(() => repo.getLockedUntilMs()).thenAnswer(
        (_) async => DateTime.now().millisecondsSinceEpoch - 30000);
    when(() => repo.verifyPin('123456')).thenAnswer((_) async => true);
    expect(
        await verifyPinWithLockout(repo, '123456'), PinVerifyOutcome.ok);
  ...
  test('PIN salah tepat kelipatan 5 → lockedOut', () async {
    ...
    expect(await verifyPinWithLockout(repo, '000000'),
        PinVerifyOutcome.lockedOut);
  });

  test('remainingLockoutSeconds → 0 saat lockedUntilMs belum pernah ada',
      () async {
    when(() => repo.getLockedUntilMs()).thenAnswer((_) async => 0);
    expect(await remainingLockoutSeconds(repo), 0);
  });

  test('remainingLockoutSeconds → 0 saat jeda sudah kedaluwarsa (bukan negatif)',
      () async {
    when(() => repo.getLockedUntilMs()).thenAnswer(
        (_) async => DateTime.now().millisecondsSinceEpoch - 5000);
    expect(await remainingLockoutSeconds(repo), 0);
  });
