...
void main() {
  late AppDatabase db; // Drift fresh = simulasi reinstall (profileCompleted=false)
  ...
  test('A2: reinstall + remote profileCompleted=true → bootstrap dulu → guard TIDAK needsProfile', () async {
    ...
    expect(status, isNot(OnboardingStatus.needsProfile),
        reason: 'profil dipulihkan dari cloud, jangan ulang (kelas bug d4de2f2)');
    expect(status, OnboardingStatus.done); // profil ok + budget ok
  });

  test('A2: bootstrap timeout → local tetap default → guard fail-safe needsProfile', () async {
    ...
    final repo = PreferencesRepositoryImpl(
      local: local, remote: remote, syncTimeout: const Duration(milliseconds: 10),
    );
    final guard = OnboardingGuard(onboardingDs: onbDs, prefsRepo: repo);
