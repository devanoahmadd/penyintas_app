...
void main() {
  ...
  test('kategori dengan limit dan spending ≤50%: status safe', () {
    final result = usecase(
      params(limits: [_foodLimit], txns: [_tx('food', 400000)]),
    );
    final food = result.categoryItems.firstWhere(
      (i) => i.category.slug == 'food',
    );
  ...
  test('kategori dengan spending 50–80%: status caution', () {
    final result = usecase(
      params(limits: [_foodLimit], txns: [_tx('food', 700000)]),
    );
    final food = result.categoryItems.firstWhere(
      (i) => i.category.slug == 'food',
    );
  ...
  test('kategori dengan spending >80%: status danger', () {
    final result = usecase(
      params(limits: [_foodLimit], txns: [_tx('food', 900000)]),
    );
    final food = result.categoryItems.firstWhere(
      (i) => i.category.slug == 'food',
    );
  ...
  test('disabled limit diperlakukan seperti tidak ada limit', () {
    ...
    final result = usecase(
      params(limits: [disabledLimit], txns: [_tx('food', 900000)]),
    );
    final food = result.categoryItems.firstWhere(
      (i) => i.category.slug == 'food',
    );
  ...
  test(
    'pace: projectedDaysLeft null dan paceStatus null saat tidak ada spending',
    () {
      final result = usecase(
        params(
          limits: [_foodLimit],
  ...
  test(
    'pace: paceStatus null saat remainingDays == 0 (edge case bulan pendek)',
    () {
      final result = usecase(
        params(
          limits: [_foodLimit],
  ...
  test('pace: entity-level dailyBurnRate dan projectedOperationalDays', () {
    ...
    final result = usecase(
      params(txns: [_tx('food', 300000)], daysElapsed: 10, remainingDays: 20),
    );
