...
class EditTransactionSheet extends StatefulWidget {
  const EditTransactionSheet({super.key, this.activeGoals = const []});
...
class _EditTransactionSheetState extends State<EditTransactionSheet> {
  ...
  void initState() {
    super.initState();
    // State is seeded in BLoC constructor — safe to read synchronously
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      final s = context.read<EditTransactionBloc>().state;
      if (s is EditTransactionInProgress && s.note.isNotEmpty) {
  ...
  void _openNumpad() {
    ...
    final amount = (bloc.state is EditTransactionInProgress)
        ? (bloc.state as EditTransactionInProgress).amount
        : 0;
    ...
    showModalBottomSheet<void>(
      ...
      builder: (ctx) => BlocProvider.value(
        value: bloc,
        child: _EditNumpadSheet(initialAmount: amount),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<EditTransactionBloc, EditTransactionState>(
      listener: (context, state) {
        if (state is EditTransactionSuccess) Navigator.of(context).pop(true);
              ...
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      _EditTypeToggle(isDark: isDark),
                      const SizedBox(height: AppSpacing.lg),
                      GestureDetector(
                        onTap: _openNumpad,
                        child: _EditNominalCard(isDark: isDark),
                      ),
...
class _EditNumpadSheet extends StatefulWidget {
  const _EditNumpadSheet({required this.initialAmount});
  final int initialAmount;

  @override
  State<_EditNumpadSheet> createState() => _EditNumpadSheetState();
}

class _EditNumpadSheetState extends State<_EditNumpadSheet> {
  late String _raw;
...
class _EditHandle extends StatelessWidget {
  const _EditHandle({required this.isDark});
  final bool isDark;
...
class _EditSectionLabel extends StatelessWidget {
  const _EditSectionLabel(this.text, {required this.isDark});
  final String text;
  final bool isDark;
...
class _EditTypeToggle extends StatelessWidget {
  const _EditTypeToggle({required this.isDark});
  final bool isDark;
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<EditTransactionBloc, EditTransactionState>(
      builder: (context, state) {
        final isExpense = state is EditTransactionInProgress
...
class _EditNominalCard extends StatelessWidget {
  const _EditNominalCard({required this.isDark});
  final bool isDark;
...
class _EditCategoryGrid extends StatelessWidget {
  const _EditCategoryGrid({required this.isDark});
  final bool isDark;
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<EditTransactionBloc, EditTransactionState>(
      builder: (context, state) {
        final selected = state is EditTransactionInProgress
            ? state.category
            : TransactionCategory.food;
...
class _EditCategoryCell extends StatelessWidget {
  const _EditCategoryCell({
  ...
  static IconData _icon(TransactionCategory cat) => switch (cat) {
  ...
  static String _label(TransactionCategory cat) => switch (cat) {
        ...
        TransactionCategory.income => 'Masuk',
      };
}

// ── Note Field ─────────────────────────────────────────────────────────────

class _EditNoteField extends StatelessWidget {
  const _EditNoteField({required this.controller, required this.isDark});
  ...
  Widget build(BuildContext context) {
    ...
    return TextField(
      ...
      onChanged: (v) =>
          context.read<EditTransactionBloc>().add(EditNoteChanged(v)),
    );
  }
}
...
class _EditDateField extends StatelessWidget {
  const _EditDateField({required this.isDark});
  final bool isDark;
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<EditTransactionBloc, EditTransactionState>(
      builder: (context, state) {
        final date = state is EditTransactionInProgress
            ? state.date
            : DateTime.now();
        final label =
...
class _EditNumpad extends StatelessWidget {
  const _EditNumpad({required this.onDigit, required this.isDark});
...
class _EditNumpadKey extends StatelessWidget {
  const _EditNumpadKey({
    required this.label,
    required this.onTap,
    required this.isDark,
  });
...
class _EditGoalPicker extends StatelessWidget {
  const _EditGoalPicker({required this.goals, required this.isDark});
...
class _EditSubmitButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<EditTransactionBloc, EditTransactionState>(
      builder: (context, state) {
        final isLoading = state is EditTransactionLoading;
        final isValid = state is EditTransactionInProgress && state.isValid;
