...
void main() {
  group('PreferencesModel', () {
    test(
      'fromFirestore memetakan SEMUA 13 field (CF-2: anti typo-mapping senyap)',
      () {
    ...
    test(
      'fromFirestore: tipe SALAH (bukan string/bool/num) → fallback aman, tak crash (T-2)',
      () {
    ...
    test('toFirestore menulis field wajib + updatedAt serverTimestamp', () {
      final map = PreferencesModel.fromEntity(
    ...
    test('toFirestore key-set == kontrak bersama (C1: anti-drift Dart↔rules)', () {
      ...
      final contract =
          jsonDecode(
                File(
                  'firestore-tests/preferences_contract.json',
                ).readAsStringSync(),
              )
      ...
      final keys = PreferencesModel.fromEntity(
        PreferencesEntity.defaults,
      ...
      expect(
        ...
        required,
        reason:
            'toFirestore() drift dari preferences_contract.json (sinkronkan model/rules/kontrak)',
      );
    });
