  ...
  group('syncOnLaunch', () {
    PreferencesModel completedRemote() => PreferencesModel.fromEntity(
          PreferencesEntity.defaults.copyWith(
            timezone: 'Europe/Moscow', currentCountry: 'RU', currentCity: 'Moscow',
            isPerantau: true, profileCompleted: true,
          ),
        );

    test('bootstrap: local belum selesai + remote selesai → seed local dari remote', () async {
      when(() => local.read())
          .thenAnswer((_) async => PreferencesEntity.defaults); // profileCompleted=false
      ...
      expect(r, const Right(unit));
      final seeded = verify(() => local.write(captureAny())).captured.single
          as PreferencesEntity;
      expect(seeded.profileCompleted, true);
      expect(seeded.timezone, 'Europe/Moscow'); // dipulihkan dari cloud
    });

    test('ratchet + DIRTY: local selesai & belum tersinkron → mirror retry (T-1)', () async {
      when(() => local.read()).thenAnswer(
          (_) async => PreferencesEntity.defaults.copyWith(profileCompleted: true));
      when(() => local.hasPendingMirror()).thenAnswer((_) async => true); // dirty
      when(() => remote.mirror(any())).thenAnswer((_) async {});

      final r = await repo.syncOnLaunch(budgetOnboardingCompleted: true);
      expect(r, const Right(unit));
      verifyNever(() => remote.fetch()); // local authoritative → tak baca remote
      verify(() => remote.mirror(any())).called(1); // dirty → retry mirror best-effort
    });

    test('T-1 anti-clobber: local selesai & CLEAN → TAK mirror (cloud tak ditimpa)', () async {
      when(() => local.read()).thenAnswer(
          (_) async => PreferencesEntity.defaults.copyWith(profileCompleted: true));
      when(() => local.hasPendingMirror()).thenAnswer((_) async => false); // clean

      final r = await repo.syncOnLaunch(budgetOnboardingCompleted: true);
      expect(r, const Right(unit));
      verifyNever(() => remote.fetch());
      verifyNever(() => remote.mirror(any())); // KRUSIAL: clean → cloud (device lain) tak ditimpa
    });

    test('smart-default akun lama: local belum + remote null + budget selesai → set profileCompleted', () async {
      when(() => local.read()).thenAnswer((_) async => PreferencesEntity.defaults);
      when(() => remote.fetch()).thenAnswer((_) async => null); // belum ada doc preferences
      when(() => local.write(any())).thenAnswer((_) async {});
      when(() => remote.mirror(any())).thenAnswer((_) async {});

      final r = await repo.syncOnLaunch(budgetOnboardingCompleted: true);
      expect(r, const Right(unit));
      final written = verify(() => local.write(captureAny())).captured.single
          as PreferencesEntity;
      expect(written.profileCompleted, true, reason: 'akun lama tak dipaksa ulang profil');
    });

    test('user baru: local belum + remote null + budget belum → TAK set profileCompleted', () async {
      when(() => local.read()).thenAnswer((_) async => PreferencesEntity.defaults);
    ...
    test('fetch timeout → local tak berubah → Right (fail-safe needsProfile)', () async {
      ...
      when(() => remote.fetch()).thenAnswer((_) async {
        await Future<void>.delayed(const Duration(seconds: 1)); // lampaui timeout
      ...
      expect(r, const Right(unit));
      verifyNever(() => local.write(any())); // bootstrap gagal → local tetap default
    });

    test('F-D1 anti-clobber: fetch GAGAL + budget selesai (akun lama) → TAK tulis, TAK mirror', () async {
      ...
      verifyNever(() => remote.mirror(any()));  // KRUSIAL: tak overwrite cloud asli
    });

    test('F-D2 lindungi parsial: remote ADA tapi belum selesai → TAK tulis, TAK mirror', () async {
      when(() => local.read()).thenAnswer((_) async => PreferencesEntity.defaults);
      when(() => remote.fetch()).thenAnswer((_) async => PreferencesModel.fromEntity(
            PreferencesEntity.defaults
                .copyWith(currentCountry: 'ID', currentCity: 'Jakarta'),
      ...
      verifyNever(() => remote.mirror(any())); // jangan timpa parsial dgn local kosong
    });

    test('F-D4 trust-boundary (Temuan 2): remote completed TAPI all-defaults (tanpa kota, zona default) → TAK seed', () async {
      when(() => local.read()).thenAnswer((_) async => PreferencesEntity.defaults);
      ...
      when(() => remote.fetch()).thenAnswer((_) async => PreferencesModel.fromFirestore(const {
            'timezone': 'Asia/Jakarta', 'baseCurrency': 'IDR', 'homeCurrency': 'IDR',
            'language': 'id', 'currentCountry': 'ID', 'homeCountry': 'ID',
            'isPerantau': false, 'profileCompleted': true, 'schemaVersion': 1,
          }));
      when(() => remote.mirror(any())).thenAnswer((_) async {});

      final r = await repo.syncOnLaunch(budgetOnboardingCompleted: false);
      expect(r, const Right(unit));
      verifyNever(() => local.write(any())); // jangan seed "completed" yg semu
    });

    test('F-D4 (Temuan 2): remote completed dgn kota asli → SEED (gerbang lolos utk profil sahih)', () async {
      when(() => local.read()).thenAnswer((_) async => PreferencesEntity.defaults);
      when(() => remote.fetch()).thenAnswer((_) async => PreferencesModel.fromFirestore(const {
            'timezone': 'Asia/Jakarta', 'baseCurrency': 'IDR', 'homeCurrency': 'IDR',
            'language': 'id', 'currentCountry': 'ID', 'currentCity': 'Jakarta',
            'homeCountry': 'ID', 'isPerantau': false, 'profileCompleted': true,
            'schemaVersion': 1,
          }));
      when(() => local.write(any())).thenAnswer((_) async {});
      when(() => remote.mirror(any())).thenAnswer((_) async {});
