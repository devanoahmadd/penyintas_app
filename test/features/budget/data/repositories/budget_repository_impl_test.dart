...
void main() {
  ...
  group('syncBudgetFromRemote', () {
    ...
    test(
      'local miss + online + remote ada — cache lokal lalu return remote',
      () async {
    ...
    test(
      'local miss + offline — return Right(null), tak sentuh remote',
      () async {
        when(() => local.getBudgetSettings()).thenAnswer((_) async => null);
  ...
  group('sync queue path #252', () {
    ...
    test(
      'saveBudgetSettings offline → path doc penuh, tanpa docId di data',
      () async {
        when(() => network.isConnected).thenAnswer((_) async => false);
        when(() => local.saveBudgetSettings(any())).thenAnswer((_) async {});
        stubQueueOk();
        ...
        expect(
          (captured[1] as Map<String, dynamic>).containsKey('docId'),
          isFalse,
        );
      },
    );
    ...
    test(
      'deleteBudgetLimit offline → path doc penuh, operation delete',
      () async {
        when(() => network.isConnected).thenAnswer((_) async => false);
